...
The original Dippy-compatible `.rippy` format is still supported. One
rule per line, no `[[rules]]` tables:
...
New configs should prefer TOML. If you have an existing `.rippy` file,
`rippy migrate` converts it automatically.
