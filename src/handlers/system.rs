...
mod tests {
    ...
    fn dmesg_read_allows() {
        ...
        let result = DMESG_HANDLER.classify(&HandlerContext::test("dmesg", &args));
        assert!(matches!(result, Classification::Allow(_)));
    }
    ...
    fn ip_addr_show_allows() {
        ...
        let result = IP_HANDLER.classify(&HandlerContext::test("ip", &args));
        assert!(matches!(result, Classification::Allow(_)));
    }
    ...
    fn ifconfig_bare_allows() {
        let args: Vec<String> = vec![];
        let result = IFCONFIG_HANDLER.classify(&HandlerContext::test("ifconfig", &args));
        assert!(matches!(result, Classification::Allow(_)));
    }
